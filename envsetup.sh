...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/opaosp/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/opaosp/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/opaosp/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^opaosp_") ; then
        OPAOSP_BUILD=$(echo -n $1 | sed -e 's/^opaosp_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $OPAOSP_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        OPAOSP_BUILD=
    fi
    export OPAOSP_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${OPAOSP_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    OPAOSP_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/opaosp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch opaosp_$target-userdebug
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the OPAOSP GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/opaosp/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/opaosp/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                bacon|opaosp|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function repopick() {
    T=$(gettop)
    $T/vendor/opaosp/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $OPAOSP_FIXUP_COMMON_OUT ]; then
...
. $ANDROID_BUILD_TOP/vendor/opaosp/build/envsetup.sh
